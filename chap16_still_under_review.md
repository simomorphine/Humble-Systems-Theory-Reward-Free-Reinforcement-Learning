...
Consider a two-armed bandit in which obtaining information is not free. Each time the agent pulls an arm, it receives a reward but also pays a computational or energetic cost.

**Environment:**

* **Arm A (known):** reward probability $p_A = 0.5$, with cost $c_A = 0.1$ per pull.
* **Arm B (unknown):** reward probability $p_B \in \lbrace 0.2,0.8 \rbrace$, with prior probability $1/2$ for each possibility, and cost $c_B = 0.2$ per pull.

Initially, the agent therefore knows that Arm A has expected reward $0.5$, while the expected reward of Arm B is also $0.5$. However, the agent does not know whether Arm B is actually poor ($p_B=0.2$) or excellent ($p_B=0.8$).

The problem is therefore not simply to choose the arm with the highest expected reward. Pulling Arm B has two effects: it produces a reward and, more importantly, provides information about the unknown environment. That information can change future decisions.

This creates a fundamental tension:

> **Exploration has a cost, but not exploring also has a potential cost.**

If the agent always chooses Arm A, it avoids the additional cost of Arm B but may permanently miss the possibility that $p_B=0.8$. If it repeatedly explores Arm B, it obtains information but spends more resources doing so. The appropriate amount of exploration depends on the value of the information relative to its cost and on how many future decisions remain.

This simple setting exposes a difficulty that appears in more general information-processing systems: **computation, information, and action are not independent.** An agent must spend resources to obtain information, and the information obtained can subsequently change the actions that it takes.

#### A cost-debt representation

We introduce the following geometric representation as a hypothesis to investigate.
...
where $c$ represents a normalized measure of accumulated or incurred computational cost, while $d$ represents a measure of epistemic debt: the agent's remaining uncertainty relevant to its decision.

For the bandit above, epistemic debt could be defined using the uncertainty in the posterior distribution over the unknown parameter $p_B$. For example, one possible choice is the entropy

$$
\psi = H(p_B \mid \text{observations}).
$$

Initially, before observing any outcomes from Arm B, the posterior assigns equal probability to $p_B=0.2$ and $p_B=0.8$, so the epistemic debt is maximal for this two-state uncertainty. As observations accumulate, the posterior may concentrate on one of the two possibilities, causing the epistemic debt to decrease.

The state of the agent can therefore be represented schematically in a cost-debt plane:

The important point is not that every trajectory must move directly toward the origin. Rather, the representation gives us a geometric space in which **resource expenditure and unresolved uncertainty can be studied simultaneously**.

This suggests the complex-valued quantity

$$
z = c + id
$$

as a possible state variable for studying the exploration-exploitation problem.

A natural question is then whether a decision rule based on the magnitude

$$
|z| = \sqrt{c^2+d^2}
$$

can produce meaningful exploration-exploitation behavior without introducing an externally chosen exploration coefficient.

For an action $a$, the relevant quantity would not simply be the current value of $|z|$, but the expected value after taking that action:

$$
\mathbb{E}[|z_{t+1}| \mid s_t,a].
$$

This leads to the research question:

> **Under what conditions can minimizing the expected cost-debt magnitude generate an exploration-exploitation trade-off automatically?**

The two-armed bandit is therefore not presented as a proof that the complex formulation solves exploration. It is a minimal test case in which the proposed geometry can be examined, compared with classical approaches, and potentially falsified.

The purpose of this example is to make the central problem concrete:

$$
\text{information has value, but obtaining information has a cost.}
$$

The subsequent sections investigate whether representing these two quantities as components of a complex-valued state provides useful mathematical structure for reinforcement learning.
...
- **Applications:** Complex Bellman equat